 ...
 *
 */
public class AmericanPutBinomialTree extends AlgoBinomialTree {
	 ...
	 * @param paras
	 */
	public AmericanPutBinomialTree(Parameters paras) {
		super(paras);
	...
	private void calculate() {
		...
		for (i = this.numIntervals - 1; i >= 0; i--)
		{
			...
			for (j = 0; j <= i; j++)
			{
				binomialTree[i][j].optionPrice = Math.max(this.strikePrice - binomialTree[i][j].stockPrice, discount * (upProb * binomialTree[i + 1][j + 1].optionPrice + downProb * binomialTree[i + 1][j].optionPrice));
			}
		}
